...
editUser(name,email,addres,about,img){
  ...
  console.log(this.userID,'======usersid');
  //  img = this.image_url
   this.http.patch('http://localhost:3000/users/'+this.userID, {name,email,addres,about,img}).subscribe((res:any) =>{
    console.log(res);
    return res.affected
...
// validateNewUser(name,email,password):Observable<any>{
//   return this.http.post('http://localhost:3000/users', {name,email,password});
// }
...
//   this.http.post(`http://localhost:3000/users/upload` , formData,{headers}).subscribe(res =>{
//   console.log(res);
  
//          this.image_url = res['filename']
//   }) ;  
// }
